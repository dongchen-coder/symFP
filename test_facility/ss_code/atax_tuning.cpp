...
/* i */
/* y_addr i 6 */
int calAddry_addr6( int i, int j) {
    int result = (i) * 8 / 64;
    return result;
}
/* Array A_addr	j i */ 
...
/* i */
/* tmp_addr j 8 */
int calAddrtmp_addr8( int i, int j) {
    int result = (j) * 8 / 64;
    return result;
}
/* Array y_addr	i */ 
...
SAMPLE:
        ...
        for ( int i = iLB0; i < 4096; i=(i + 1)) {
            if (cntStart == true) {
                ...
                if ( calAddry_addr0( i) == calAddry_addr0(i_Start)) {
                        ...
                        function<uint64_t(uint64_t)> sinkAddrCal = bind(calAddry_addr0, _1);
                        /* i 1 */
...
#endif
#ifdef DEBUG
                            cout << "[y_addr0 --> y_addr0] [" << parallel_rt << "] (" << i_Start<< ") --> (" << i<< ") " << endl;
#endif
                            // cout << "y_addr0;" << "(" << i_Start<< ");" << "y_addr0;" << "(" << i<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                    goto EndSample;
                }
            }
            cntStart = true;
        ...
        for ( int i = iLB3; i < 4096; i=(i + 1)) {
            ...
            for ( int j = jLB4; j < 4096; j=(j + 1)) {
...
#endif
#ifdef DEBUG
                                cout << "[y_addr0 --> y_addr6] [" << parallel_rt << "] (" << i_Start<< ") --> (" << i<< ", " << j<< ") " << endl;
#endif
                                // cout << "y_addr0;" << "(" << i_Start<< ");" << "y_addr6;" << "(" << i<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                        goto EndSample;
                    }
                }
                if (cntStart == true) cnt++;
                ...
                if (cntStart == true) {
                    cnt++;
                    if ( calAddry_addr9( i, j) == calAddry_addr0(i_Start)) {
                            /* is_src_loop_outermost: 1 */
                            /* is_sink_loop_outermost: 1 */
                            /* is_normal_ref: false */
                            /* is_in_same_loop: false */
...
#endif
#ifdef DEBUG
                                cout << "[y_addr0 --> y_addr9] [" << parallel_rt << "] (" << i_Start<< ") --> (" << i<< ", " << j<< ") " << endl;
#endif
                                // cout << "y_addr0;" << "(" << i_Start<< ");" << "y_addr9;" << "(" << i<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                        goto EndSample;
                    }
                }
            }
...
SAMPLE:
        ...
        for ( int i = iLB3; i < 4096; i=(i + 1)) {
            ...
            for ( int j = jLB4; j < 4096; j=(j + 1)) {
...
#endif
#ifdef DEBUG
                                cout << "[y_addr6 --> y_addr6] [" << parallel_rt << "] (" << i_Start<< ", " << j_Start<< ") --> (" << i<< ", " << j<< ") " << endl;
#endif
                                // cout << "y_addr6;" << "(" << i_Start<< ", " << j_Start<< ");" << "y_addr6;" << "(" << i<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                        goto EndSample;
                    }
                }
                cntStart = true;
                ...
                if (cntStart == true) {
                    cnt++;
                    if ( calAddry_addr9( i, j) == calAddry_addr6(i_Start, j_Start)) {
                            /* is_src_loop_outermost: 1 */
                            /* is_sink_loop_outermost: 1 */
                            /* is_normal_ref: false */
                            /* is_in_same_loop: true */
...
#endif
#ifdef DEBUG
                                cout << "[y_addr6 --> y_addr9] [" << parallel_rt << "] (" << i_Start<< ", " << j_Start<< ") --> (" << i<< ", " << j<< ") " << endl;
#endif
                                // cout << "y_addr6;" << "(" << i_Start<< ", " << j_Start<< ");" << "y_addr9;" << "(" << i<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                        goto EndSample;
                    }
                }
            }
...
SAMPLE:
        ...
        for ( int i = iLB1; i < 4096; i=(i + 1)) {
...
#endif
#ifdef DEBUG
                            cout << "[tmp_addr1 --> tmp_addr1] [" << parallel_rt << "] (" << i_Start<< ") --> (" << i<< ") " << endl;
#endif
                            // cout << "tmp_addr1;" << "(" << i_Start<< ");" << "tmp_addr1;" << "(" << i<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                    goto EndSample;
                }
            }
            cntStart = true;
            ...
            for ( int j = jLB2; j < 4096; j=(j + 1)) {
...
#endif
#ifdef DEBUG
                                cout << "[tmp_addr1 --> tmp_addr2] [" << parallel_rt << "] (" << i_Start<< ") --> (" << i<< ", " << j<< ") " << endl;
#endif
                                // cout << "tmp_addr1;" << "(" << i_Start<< ");" << "tmp_addr2;" << "(" << i<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                        goto EndSample;
                    }
                }
                if (cntStart == true) cnt++;
                ...
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrtmp_addr5( i, j) == calAddrtmp_addr1(i_Start)) {
                            /* is_src_loop_outermost: 1 */
                            /* is_sink_loop_outermost: 1 */
                            /* is_normal_ref: false */
                            /* is_in_same_loop: true */
...
#endif
#ifdef DEBUG
                                cout << "[tmp_addr1 --> tmp_addr5] [" << parallel_rt << "] (" << i_Start<< ") --> (" << i<< ", " << j<< ") " << endl;
#endif
                                // cout << "tmp_addr1;" << "(" << i_Start<< ");" << "tmp_addr5;" << "(" << i<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                        goto EndSample;
                    }
                }
            }
        ...
        for ( int i = iLB3; i < 4096; i=(i + 1)) {
            ...
            for ( int j = jLB4; j < 4096; j=(j + 1)) {
                ...
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrtmp_addr8( i, j) == calAddrtmp_addr1(i_Start)) {
                            /* is_src_loop_outermost: 1 */
                            /* is_sink_loop_outermost: 1 */
                            /* is_normal_ref: false */
                            /* is_in_same_loop: false */
...
#endif
#ifdef DEBUG
                                cout << "[tmp_addr1 --> tmp_addr8] [" << parallel_rt << "] (" << i_Start<< ") --> (" << i<< ", " << j<< ") " << endl;
#endif
                                // cout << "tmp_addr1;" << "(" << i_Start<< ");" << "tmp_addr8;" << "(" << i<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                        goto EndSample;
                    }
                }
                if (cntStart == true) cnt++;
...
SAMPLE:
        ...
        for ( int i = iLB1; i < 4096; i=(i + 1)) {
...
#endif
#ifdef DEBUG
                            cout << "[tmp_addr2 --> tmp_addr1] [" << parallel_rt << "] (" << i_Start<< ", " << j_Start<< ") --> (" << i<< ") " << endl;
#endif
                            // cout << "tmp_addr2;" << "(" << i_Start<< ", " << j_Start<< ");" << "tmp_addr1;" << "(" << i<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                    goto EndSample;
                }
            }
            {
            ...
            for ( int j = jLB2; j < 4096; j=(j + 1)) {
...
#endif
#ifdef DEBUG
                                cout << "[tmp_addr2 --> tmp_addr2] [" << parallel_rt << "] (" << i_Start<< ", " << j_Start<< ") --> (" << i<< ", " << j<< ") " << endl;
#endif
                                // cout << "tmp_addr2;" << "(" << i_Start<< ", " << j_Start<< ");" << "tmp_addr2;" << "(" << i<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                        goto EndSample;
                    }
                }
                cntStart = true;
                ...
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrtmp_addr5( i, j) == calAddrtmp_addr2(i_Start, j_Start)) {
                            /* is_src_loop_outermost: 1 */
                            /* is_sink_loop_outermost: 1 */
                            /* is_normal_ref: false */
                            /* is_in_same_loop: true */
...
#endif
#ifdef DEBUG
                                cout << "[tmp_addr2 --> tmp_addr5] [" << parallel_rt << "] (" << i_Start<< ", " << j_Start<< ") --> (" << i<< ", " << j<< ") " << endl;
#endif
                                // cout << "tmp_addr2;" << "(" << i_Start<< ", " << j_Start<< ");" << "tmp_addr5;" << "(" << i<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                        goto EndSample;
                    }
                }
            }
        ...
        for ( int i = iLB3; i < 4096; i=(i + 1)) {
            ...
            for ( int j = jLB4; j < 4096; j=(j + 1)) {
                ...
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrtmp_addr8( i, j) == calAddrtmp_addr2(i_Start, j_Start)) {
                            /* is_src_loop_outermost: 1 */
                            /* is_sink_loop_outermost: 1 */
                            /* is_normal_ref: false */
                            /* is_in_same_loop: false */
...
#endif
#ifdef DEBUG
                                cout << "[tmp_addr2 --> tmp_addr8] [" << parallel_rt << "] (" << i_Start<< ", " << j_Start<< ") --> (" << i<< ", " << j<< ") " << endl;
#endif
                                // cout << "tmp_addr2;" << "(" << i_Start<< ", " << j_Start<< ");" << "tmp_addr8;" << "(" << i<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                        goto EndSample;
                    }
                }
                if (cntStart == true) cnt++;
...
SAMPLE:
        ...
        for ( int i = iLB3; i < 4096; i=(i + 1)) {
            ...
            for ( int j = jLB4; j < 4096; j=(j + 1)) {
                ...
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr7( i, j) == calAddrA_addr3(i_Start, j_Start)) {
                            /* is_src_loop_outermost: 1 */
                            /* is_sink_loop_outermost: 1 */
                            /* is_normal_ref: false */
                            /* is_in_same_loop: false */
...
#endif
#ifdef DEBUG
                                cout << "[A_addr3 --> A_addr7] [" << parallel_rt << "] (" << i_Start<< ", " << j_Start<< ") --> (" << i<< ", " << j<< ") " << endl;
#endif
                                // cout << "A_addr3;" << "(" << i_Start<< ", " << j_Start<< ");" << "A_addr7;" << "(" << i<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                        goto EndSample;
                    }
                }
                if (cntStart == true) cnt++;
...
#endif
#ifdef DEBUG
                                cout << "[x_addr4 --> x_addr4] [" << parallel_rt << "] (" << i_Start<< ", " << j_Start<< ") --> (" << i<< ", " << j<< ") " << endl;
#endif
                                // cout << "x_addr4;" << "(" << i_Start<< ", " << j_Start<< ");" << "x_addr4;" << "(" << i<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                        goto EndSample;
                    }
                }
                cntStart = true;
...
SAMPLE:
...
#endif
#ifdef DEBUG
                            cout << "[tmp_addr5 --> tmp_addr1] [" << parallel_rt << "] (" << i_Start<< ", " << j_Start<< ") --> (" << i<< ") " << endl;
#endif
                            // cout << "tmp_addr5;" << "(" << i_Start<< ", " << j_Start<< ");" << "tmp_addr1;" << "(" << i<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                    goto EndSample;
                }
            }
            {
...
#endif
#ifdef DEBUG
                                cout << "[tmp_addr5 --> tmp_addr2] [" << parallel_rt << "] (" << i_Start<< ", " << j_Start<< ") --> (" << i<< ", " << j<< ") " << endl;
#endif
                                // cout << "tmp_addr5;" << "(" << i_Start<< ", " << j_Start<< ");" << "tmp_addr2;" << "(" << i<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                        goto EndSample;
                    }
                }
                if (cntStart == true) cnt++;
        ...
        for ( int i = iLB3; i < 4096; i=(i + 1)) {
            ...
            for ( int j = jLB4; j < 4096; j=(j + 1)) {
                ...
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrtmp_addr8( i, j) == calAddrtmp_addr5(i_Start, j_Start)) {
                            /* is_src_loop_outermost: 1 */
                            /* is_sink_loop_outermost: 1 */
                            /* is_normal_ref: false */
                            /* is_in_same_loop: false */
...
#endif
#ifdef DEBUG
                                cout << "[tmp_addr5 --> tmp_addr8] [" << parallel_rt << "] (" << i_Start<< ", " << j_Start<< ") --> (" << i<< ", " << j<< ") " << endl;
#endif
                                // cout << "tmp_addr5;" << "(" << i_Start<< ", " << j_Start<< ");" << "tmp_addr8;" << "(" << i<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                        goto EndSample;
                    }
                }
                if (cntStart == true) cnt++;
...
SAMPLE:
        ...
        for ( int i = iLB3; i < 4096; i=(i + 1)) {
            ...
            for ( int j = jLB4; j < 4096; j=(j + 1)) {
                ...
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrtmp_addr8( i, j) == calAddrtmp_addr8(i_Start, j_Start)) {
                            /* is_src_loop_outermost: 1 */
                            /* is_sink_loop_outermost: 1 */
                            /* is_normal_ref: false */
                            /* is_in_same_loop: true */
...
#endif
#ifdef DEBUG
                                cout << "[tmp_addr8 --> tmp_addr8] [" << parallel_rt << "] (" << i_Start<< ", " << j_Start<< ") --> (" << i<< ", " << j<< ") " << endl;
#endif
                                // cout << "tmp_addr8;" << "(" << i_Start<< ", " << j_Start<< ");" << "tmp_addr8;" << "(" << i<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                        goto EndSample;
                    }
                }
                cntStart = true;
...
#endif
#ifdef DEBUG
                                cout << "[y_addr9 --> y_addr6] [" << parallel_rt << "] (" << i_Start<< ", " << j_Start<< ") --> (" << i<< ", " << j<< ") " << endl;
#endif
                                // cout << "y_addr9;" << "(" << i_Start<< ", " << j_Start<< ");" << "y_addr6;" << "(" << i<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                        goto EndSample;
                    }
                }
                if (cntStart == true) cnt++;
...
void generate_per_thread_reuse(int thread_cnt) {
    ...
    ref_y_addr0(thread_cnt, tmpRT);
    ref_y_addr6(thread_cnt, tmpRT);
    ref_tmp_addr1(thread_cnt, tmpRT);
    ref_tmp_addr2(thread_cnt, tmpRT);
    ref_A_addr3(thread_cnt, tmpRT);
    ref_x_addr4(thread_cnt, tmpRT);
    ref_tmp_addr5(thread_cnt, tmpRT);
    ref_A_addr7(thread_cnt, tmpRT);
    ref_tmp_addr8(thread_cnt, tmpRT);
