...
SAMPLE:
        ...
        {
        int iLB0 = i_Start;
        for ( int i = iLB0; i < 1023; i=(i + 1)) {
            {
            int jLB1 = 1;
            if ( iLB0 == i_Start ) {
                jLB1 = j_Start;
            }
            for ( int j = jLB1; j < 1023; j=(j + 1)) {
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr0( i, j) == calAddrA_addr3(i_Start, j_Start)) {
                            uint64_t parallel_rt = parallel_predict((i_Start -1), i, cnt, 10444840, 10444840, true);
