...
SAMPLE:
        ...
        for ( int i = iLB0; i < 8192; i=(i + 1)) {
            ...
            if (cntStart == true) {
                cnt++;
                int addr = calAddry_addr0( i);
                Sample iter("y_addr0", {i});
        ...
        for ( int i = iLB3; i < 8192; i=(i + 1)) {
            ...
            for ( int j = jLB4; j < 8192; j=(j + 1)) {
                if (cntStart == true) {
                    cnt++;
                    int addr = calAddry_addr6( i, j);
                    Sample iter("y_addr6", {i, j});
                    if (LAT.find(addr) != LAT.end()) {
                            /* Find a reuse. Find the Sample object of the reuse src */
                            uint64_t ris = cnt - LAT[addr];
                            Sample srcSample = LATSampleIterMap[LAT[addr]];
                ...
                if (cntStart == true) {
                    cnt++;
                    int addr = calAddry_addr9( i, j);
                    Sample iter("y_addr9", {i, j});
                    if (LAT.find(addr) != LAT.end()) {
                            /* Find a reuse. Find the Sample object of the reuse src */
                            uint64_t ris = cnt - LAT[addr];
                            Sample srcSample = LATSampleIterMap[LAT[addr]];
...
SAMPLE:
        ...
        if ( record.find(idx_string) != record.end() ) goto SAMPLE;
        record.insert( idx_string );
        Sample sample("tmp_addr1", {iSample});
#if defined(INTERLEAVE_DEBUG) || defined(DEBUG)
        cout << "Samples:" << sample.toString() << endl;
#endif
        samples[sample] = 1;
        ...
        for ( int i = iLB1; i < 8192; i=(i + 1)) {
            ...
            if (cntStart == true) {
                cnt++;
                int addr = calAddrtmp_addr1( i);
                Sample iter("tmp_addr1", {i});
                if (LAT.find(addr) != LAT.end()) {
                        /* Find a reuse. Find the Sample object of the reuse src */
                        uint64_t ris = cnt - LAT[addr];
                        Sample srcSample = LATSampleIterMap[LAT[addr]];
            ...
            for ( int j = jLB2; j < 8192; j=(j + 1)) {
                if (cntStart == true) {
                    cnt++;
                    int addr = calAddrtmp_addr2( i, j);
                    Sample iter("tmp_addr2", {i, j});
                    if (LAT.find(addr) != LAT.end()) {
                            /* Find a reuse. Find the Sample object of the reuse src */
                            uint64_t ris = cnt - LAT[addr];
                            Sample srcSample = LATSampleIterMap[LAT[addr]];
                ...
                if (cntStart == true) {
                    cnt++;
                    int addr = calAddrtmp_addr5( i, j);
                    Sample iter("tmp_addr5", {i, j});
                    if (LAT.find(addr) != LAT.end()) {
                            /* Find a reuse. Find the Sample object of the reuse src */
                            uint64_t ris = cnt - LAT[addr];
                            Sample srcSample = LATSampleIterMap[LAT[addr]];
        ...
        for ( int i = iLB3; i < 8192; i=(i + 1)) {
            ...
            for ( int j = jLB4; j < 8192; j=(j + 1)) {
                ...
                if (cntStart == true) {
                    cnt++;
                    int addr = calAddrtmp_addr8( i, j);
                    Sample iter("tmp_addr8", {i, j});
                    if (LAT.find(addr) != LAT.end()) {
                            /* Find a reuse. Find the Sample object of the reuse src */
                            uint64_t ris = cnt - LAT[addr];
                            Sample srcSample = LATSampleIterMap[LAT[addr]];
...
SAMPLE:
        ...
        if ( record.find(idx_string) != record.end() ) goto SAMPLE;
        record.insert( idx_string );
        Sample sample("tmp_addr2", {iSample, jSample});
#if defined(INTERLEAVE_DEBUG) || defined(DEBUG)
        cout << "Samples:" << sample.toString() << endl;
#endif
        samples[sample] = 1;
        ...
        for ( int i = iLB1; i < 8192; i=(i + 1)) {
            if (cntStart == true) {
                ...
                if (LAT.find(addr) != LAT.end()) {
                        ...
                        /* is_in_same_loop: true */
                        /* register the src/sink addr calculation function. Calling this function in other function do not have to know all index variables */
                        function<uint64_t(uint64_t)> srcAddrCal = bind(calAddrtmp_addr2, _1, srcSample.ivs[1]);
                        function<uint64_t(uint64_t)> sinkAddrCal = bind(calAddrtmp_addr1, _1);
                        /* i 32769 */
                        /* compute the number of accesses between source and sink chunk */
                        uint64_t middle_accesses = 0;
...
SAMPLE:
        ...
        for ( int i = iLB3; i < 8192; i=(i + 1)) {
            ...
            for ( int j = jLB4; j < 8192; j=(j + 1)) {
                ...
                if (cntStart == true) {
                    cnt++;
                    int addr = calAddrA_addr7( i, j);
                    Sample iter("A_addr7", {i, j});
                    if (LAT.find(addr) != LAT.end()) {
                            /* Find a reuse. Find the Sample object of the reuse src */
                            uint64_t ris = cnt - LAT[addr];
                            Sample srcSample = LATSampleIterMap[LAT[addr]];
...
SAMPLE:
        ...
        if ( record.find(idx_string) != record.end() ) goto SAMPLE;
        record.insert( idx_string );
        Sample sample("x_addr4", {iSample, jSample});
#if defined(INTERLEAVE_DEBUG) || defined(DEBUG)
        cout << "Samples:" << sample.toString() << endl;
#endif
        samples[sample] = 1;
        ...
        for ( int i = iLB1; i < 8192; i=(i + 1)) {
            ...
            for ( int j = jLB2; j < 8192; j=(j + 1)) {
                ...
                if (cntStart == true) {
                    cnt++;
                    int addr = calAddrx_addr4( i, j);
                    Sample iter("x_addr4", {i, j});
                    if (LAT.find(addr) != LAT.end()) {
                            /* Find a reuse. Find the Sample object of the reuse src */
                            uint64_t ris = cnt - LAT[addr];
                            Sample srcSample = LATSampleIterMap[LAT[addr]];
                            ...
                            /* is_in_same_loop: true */
                            /* register the src/sink addr calculation function. Calling this function in other function do not have to know all index variables */
                            function<uint64_t(uint64_t)> srcAddrCal = bind(calAddrx_addr4, _1, srcSample.ivs[1]);
...
SAMPLE:
        ...
        if ( record.find(idx_string) != record.end() ) goto SAMPLE;
        record.insert( idx_string );
        Sample sample("tmp_addr5", {iSample, jSample});
#if defined(INTERLEAVE_DEBUG) || defined(DEBUG)
        cout << "Samples:" << sample.toString() << endl;
#endif
        samples[sample] = 1;
        ...
        for ( int i = iLB1; i < 8192; i=(i + 1)) {
            if (cntStart == true) {
                ...
                if (LAT.find(addr) != LAT.end()) {
                        ...
                        /* is_in_same_loop: true */
                        /* register the src/sink addr calculation function. Calling this function in other function do not have to know all index variables */
                        function<uint64_t(uint64_t)> srcAddrCal = bind(calAddrtmp_addr5, _1, srcSample.ivs[1]);
                        function<uint64_t(uint64_t)> sinkAddrCal = bind(calAddrtmp_addr1, _1);
                        /* i 32769 */
                        /* compute the number of accesses between source and sink chunk */
                        uint64_t middle_accesses = 0;
...
SAMPLE:
        ...
        if ( record.find(idx_string) != record.end() ) goto SAMPLE;
        record.insert( idx_string );
        Sample sample("y_addr6", {iSample, jSample});
#if defined(INTERLEAVE_DEBUG) || defined(DEBUG)
        cout << "Samples:" << sample.toString() << endl;
#endif
        samples[sample] = 1;
            ...
            for ( int j = jLB4; j < 8192; j=(j + 1)) {
                ...
                if (cntStart == true) {
                    ...
                    if (LAT.find(addr) != LAT.end()) {
                            ...
                            /* is_in_same_loop: true */
                            /* register the src/sink addr calculation function. Calling this function in other function do not have to know all index variables */
                            function<uint64_t(uint64_t)> srcAddrCal = bind(calAddry_addr6, _1, srcSample.ivs[1]);
                            function<uint64_t(uint64_t)> sinkAddrCal = bind(calAddry_addr6, _1, j);
                            /* i 32768 */
                            /* j 4 */
                            /* compute the number of accesses between source and sink chunk */
...
SAMPLE:
        ...
        if ( record.find(idx_string) != record.end() ) goto SAMPLE;
        record.insert( idx_string );
        Sample sample("A_addr7", {iSample, jSample});
#if defined(INTERLEAVE_DEBUG) || defined(DEBUG)
        cout << "Samples:" << sample.toString() << endl;
#endif
        samples[sample] = 1;
            ...
            for ( int j = jLB4; j < 8192; j=(j + 1)) {
                ...
                if (cntStart == true) {
                    ...
                    if (LAT.find(addr) != LAT.end()) {
                            ...
                            /* is_in_same_loop: true */
                            /* register the src/sink addr calculation function. Calling this function in other function do not have to know all index variables */
                            function<uint64_t(uint64_t)> srcAddrCal = bind(calAddrA_addr7, _1, srcSample.ivs[1]);
                            function<uint64_t(uint64_t)> sinkAddrCal = bind(calAddrA_addr7, _1, j);
                            /* i 32768 */
                            /* j 4 */
                            /* compute the number of accesses between source and sink chunk */
...
SAMPLE:
        ...
        if ( record.find(idx_string) != record.end() ) goto SAMPLE;
        record.insert( idx_string );
        Sample sample("tmp_addr8", {iSample, jSample});
#if defined(INTERLEAVE_DEBUG) || defined(DEBUG)
        cout << "Samples:" << sample.toString() << endl;
#endif
        samples[sample] = 1;
            ...
            for ( int j = jLB4; j < 8192; j=(j + 1)) {
                ...
                if (cntStart == true) {
                    ...
                    if (LAT.find(addr) != LAT.end()) {
                            ...
                            /* is_in_same_loop: true */
                            /* register the src/sink addr calculation function. Calling this function in other function do not have to know all index variables */
                            function<uint64_t(uint64_t)> srcAddrCal = bind(calAddrtmp_addr8, _1, srcSample.ivs[1]);
                            function<uint64_t(uint64_t)> sinkAddrCal = bind(calAddrtmp_addr8, _1, j);
                            /* i 32768 */
                            /* j 4 */
                            /* compute the number of accesses between source and sink chunk */
...
SAMPLE:
        ...
        if ( record.find(idx_string) != record.end() ) goto SAMPLE;
        record.insert( idx_string );
        Sample sample("y_addr9", {iSample, jSample});
#if defined(INTERLEAVE_DEBUG) || defined(DEBUG)
        cout << "Samples:" << sample.toString() << endl;
#endif
        samples[sample] = 1;
            ...
            for ( int j = jLB4; j < 8192; j=(j + 1)) {
                if (cntStart == true) {
                    ...
                    if (LAT.find(addr) != LAT.end()) {
                            ...
                            /* is_in_same_loop: true */
                            /* register the src/sink addr calculation function. Calling this function in other function do not have to know all index variables */
                            function<uint64_t(uint64_t)> srcAddrCal = bind(calAddry_addr9, _1, srcSample.ivs[1]);
                            function<uint64_t(uint64_t)> sinkAddrCal = bind(calAddry_addr6, _1, j);
                            /* i 32768 */
                            /* j 4 */
                            /* compute the number of accesses between source and sink chunk */
...
int main() {
    ...
    PAPI_timer_start();
#endif
    thread t_y_addr_0(ref_y_addr0);
    thread t_tmp_addr_1(ref_tmp_addr1);
    thread t_tmp_addr_2(ref_tmp_addr2);
    thread t_A_addr_3(ref_A_addr3);
    thread t_x_addr_4(ref_x_addr4);
    thread t_tmp_addr_5(ref_tmp_addr5);
    thread t_y_addr_6(ref_y_addr6);
    thread t_A_addr_7(ref_A_addr7);
    thread t_tmp_addr_8(ref_tmp_addr8);
    thread t_y_addr_9(ref_y_addr9);
    t_y_addr_0.join();
    t_tmp_addr_1.join();
    t_tmp_addr_2.join();
    t_A_addr_3.join();
