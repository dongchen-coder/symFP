...
/* for (j, 0, 4096) */
    ...
    uint64_t access;
        ...
        chunk_num = (4096 - 0) % (THREAD_NUM * chunk_size) == 0 ? (4096 - 0) / (THREAD_NUM * chunk_size) : (4096 - 0) / (THREAD_NUM * chunk_size) + 1;
#else
        chunk_num = 1;
        chunk_size = (4096 - 0) / THREAD_NUM;
#endif
            ...
            vector<int> thread_pool;
                ...
                while ( !candidate_thread_pool.empty()) {
                    ...
                    for (int t_select = 0; t_select < THREAD_NUM; t_select++) {
                        ...
                        /* 0*/
                        progress[t_select][0] = progress[t_select][0] + (progress[t_select][1] / 4096);
                        progress[t_select][1] = progress[t_select][1] % 4096;
#ifdef DEBUG
