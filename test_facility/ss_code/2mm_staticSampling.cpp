...
void ref_D_addr0() {
    ...
    uint64_t prev_cnt_D_addr1 = -1;
    uint64_t prev_i_Start_D_addr1 = -1;
    uint64_t prev_i_End_D_addr1 = -1;
    uint64_t prev_j_Start_D_addr1 = -1;
    uint64_t prev_j_End_D_addr1 = -1;
    /* Generating sampling loop */
    set<string> record;
    for ( int s = 0; s < 104;) {
SAMPLE:
        ...
        if ( prev_cnt_D_addr0 != -1) {
            if ( calAddrD_addr0( i_Start - prev_i_Start_D_addr0 + prev_i_End_D_addr0, j_Start - prev_j_Start_D_addr0 + prev_j_End_D_addr0) == calAddrD_addr0(i_Start, j_Start)) {
                rtHistoCal(prev_cnt_D_addr0);
                goto EndSample;
            }
        }
        if ( prev_cnt_D_addr1 != -1) {
            if ( calAddrD_addr1( i_Start - prev_i_Start_D_addr1 + prev_i_End_D_addr1, j_Start - prev_j_Start_D_addr1 + prev_j_End_D_addr1) == calAddrD_addr0(i_Start, j_Start)) {
                rtHistoCal(prev_cnt_D_addr1);
                goto EndSample;
            }
        }
        /* Generating reuse search code */
            ...
            for ( int j = jLB4; j < 1024; j++) {
                ...
                if (cntStart == true) {
                    ...
                    if ( calAddrD_addr1( i, j) == calAddrD_addr0(i_Start, j_Start)) {
                        rtHistoCal(cnt);
                        prev_cnt_D_addr1 = cnt;
                        prev_i_Start_D_addr1 = i_Start;
                        prev_i_End_D_addr1 = i;
                        prev_j_Start_D_addr1 = j_Start;
                        prev_j_End_D_addr1 = j;
                        goto EndSample;
                    }
                }
                {
...
SAMPLE:
        ...
        /* Generating search reuse body code (use reuse are in the same loop) */
        if ( prev_cnt_D_addr0 != -1) {
            if ( calAddrD_addr0( i_Start - prev_i_Start_D_addr0 + prev_i_End_D_addr0, j_Start - prev_j_Start_D_addr0 + prev_j_End_D_addr0) == calAddrD_addr1(i_Start, j_Start)) {
                rtHistoCal(prev_cnt_D_addr0);
                goto EndSample;
            }
        }
        if ( prev_cnt_D_addr1 != -1) {
            if ( calAddrD_addr1( i_Start - prev_i_Start_D_addr1 + prev_i_End_D_addr1, j_Start - prev_j_Start_D_addr1 + prev_j_End_D_addr1) == calAddrD_addr1(i_Start, j_Start)) {
                rtHistoCal(prev_cnt_D_addr1);
                goto EndSample;
            }
        }
...
void ref_D_addr2() {
    /* Generating search reuse init code (same loop) */
    uint64_t prev_cnt_D_addr2 = -1;
    uint64_t prev_i_Start_D_addr2 = -1;
    uint64_t prev_i_End_D_addr2 = -1;
    uint64_t prev_j_Start_D_addr2 = -1;
    uint64_t prev_j_End_D_addr2 = -1;
    uint64_t prev_k_Start_D_addr2 = -1;
    uint64_t prev_k_End_D_addr2 = -1;
    uint64_t prev_cnt_D_addr3 = -1;
    uint64_t prev_i_Start_D_addr3 = -1;
    uint64_t prev_i_End_D_addr3 = -1;
    uint64_t prev_j_Start_D_addr3 = -1;
    uint64_t prev_j_End_D_addr3 = -1;
    uint64_t prev_k_Start_D_addr3 = -1;
    uint64_t prev_k_End_D_addr3 = -1;
    /* Generating sampling loop */
    set<string> record;
    for ( int s = 0; s < 1073;) {
SAMPLE:
        ...
        /* Generating search reuse body code (use reuse are in the same loop) */
        if ( prev_cnt_D_addr2 != -1) {
            if ( calAddrD_addr2( i_Start - prev_i_Start_D_addr2 + prev_i_End_D_addr2, j_Start - prev_j_Start_D_addr2 + prev_j_End_D_addr2, k_Start - prev_k_Start_D_addr2 + prev_k_End_D_addr2) == calAddrD_addr2(i_Start, j_Start, k_Start)) {
                rtHistoCal(prev_cnt_D_addr2);
                goto EndSample;
            }
        }
        if ( prev_cnt_D_addr3 != -1) {
            if ( calAddrD_addr3( i_Start - prev_i_Start_D_addr3 + prev_i_End_D_addr3, j_Start - prev_j_Start_D_addr3 + prev_j_End_D_addr3, k_Start - prev_k_Start_D_addr3 + prev_k_End_D_addr3) == calAddrD_addr2(i_Start, j_Start, k_Start)) {
                rtHistoCal(prev_cnt_D_addr3);
                goto EndSample;
            }
        }
        /* Generating reuse search code */
                ...
                for ( int k = kLB5; k < 1024; k++) {
                    ...
                    if (cntStart == true) {
                        ...
                        if ( calAddrD_addr2( i, j, k) == calAddrD_addr2(i_Start, j_Start, k_Start)) {
                            rtHistoCal(cnt);
                            prev_cnt_D_addr2 = cnt;
                            prev_i_Start_D_addr2 = i_Start;
                            prev_i_End_D_addr2 = i;
                            prev_j_Start_D_addr2 = j_Start;
                            prev_j_End_D_addr2 = j;
                            prev_k_Start_D_addr2 = k_Start;
                            prev_k_End_D_addr2 = k;
                            goto EndSample;
                        }
                    }
                    cntStart = true;
                    if (cntStart == true) {
                        ...
                        if ( calAddrD_addr3( i, j, k) == calAddrD_addr2(i_Start, j_Start, k_Start)) {
                            rtHistoCal(cnt);
                            prev_cnt_D_addr3 = cnt;
                            prev_i_Start_D_addr3 = i_Start;
                            prev_i_End_D_addr3 = i;
                            prev_j_Start_D_addr3 = j_Start;
                            prev_j_End_D_addr3 = j;
                            prev_k_Start_D_addr3 = k_Start;
                            prev_k_End_D_addr3 = k;
                            goto EndSample;
                        }
                    }
                }
...
SAMPLE:
        ...
        /* Generating search reuse body code (use reuse are in the same loop) */
        if ( prev_cnt_D_addr2 != -1) {
            if ( calAddrD_addr2( i_Start - prev_i_Start_D_addr2 + prev_i_End_D_addr2, j_Start - prev_j_Start_D_addr2 + prev_j_End_D_addr2, k_Start - prev_k_Start_D_addr2 + prev_k_End_D_addr2) == calAddrD_addr3(i_Start, j_Start, k_Start)) {
                rtHistoCal(prev_cnt_D_addr2);
                goto EndSample;
            }
        }
        if ( prev_cnt_D_addr3 != -1) {
            if ( calAddrD_addr3( i_Start - prev_i_Start_D_addr3 + prev_i_End_D_addr3, j_Start - prev_j_Start_D_addr3 + prev_j_End_D_addr3, k_Start - prev_k_Start_D_addr3 + prev_k_End_D_addr3) == calAddrD_addr3(i_Start, j_Start, k_Start)) {
                rtHistoCal(prev_cnt_D_addr3);
                goto EndSample;
            }
        }
...
void ref_tmp_addr1() {
    ...
    uint64_t prev_k_Start_tmp_addr1 = -1;
    uint64_t prev_k_End_tmp_addr1 = -1;
    ...
    uint64_t prev_k_Start_tmp_addr2 = -1;
    uint64_t prev_k_End_tmp_addr2 = -1;
    /* Generating sampling loop */
    set<string> record;
    for ( int s = 0; s < 1073;) {
SAMPLE:
        ...
        if ( prev_cnt_tmp_addr1 != -1) {
            if ( calAddrtmp_addr1( i_Start - prev_i_Start_tmp_addr1 + prev_i_End_tmp_addr1, j_Start - prev_j_Start_tmp_addr1 + prev_j_End_tmp_addr1, k_Start - prev_k_Start_tmp_addr1 + prev_k_End_tmp_addr1) == calAddrtmp_addr1(i_Start, j_Start, k_Start)) {
                rtHistoCal(prev_cnt_tmp_addr1);
                goto EndSample;
            }
        }
        if ( prev_cnt_tmp_addr2 != -1) {
            if ( calAddrtmp_addr2( i_Start - prev_i_Start_tmp_addr2 + prev_i_End_tmp_addr2, j_Start - prev_j_Start_tmp_addr2 + prev_j_End_tmp_addr2, k_Start - prev_k_Start_tmp_addr2 + prev_k_End_tmp_addr2) == calAddrtmp_addr1(i_Start, j_Start, k_Start)) {
                ...
                for ( int k = kLB2; k < 1024; k++) {
                    ...
                    if (cntStart == true) {
                        ...
                        if ( calAddrtmp_addr1( i, j, k) == calAddrtmp_addr1(i_Start, j_Start, k_Start)) {
                            ...
                            prev_k_Start_tmp_addr1 = k_Start;
                            prev_k_End_tmp_addr1 = k;
                            goto EndSample;
                        }
                    }
                    cntStart = true;
                    if (cntStart == true) {
                        ...
                        if ( calAddrtmp_addr2( i, j, k) == calAddrtmp_addr1(i_Start, j_Start, k_Start)) {
                            ...
                            prev_k_Start_tmp_addr2 = k_Start;
                            prev_k_End_tmp_addr2 = k;
                            goto EndSample;
                        }
                    }
                }
...
SAMPLE:
        ...
        /* Generating search reuse body code (use reuse are in the same loop) */
        if ( prev_cnt_tmp_addr1 != -1) {
            if ( calAddrtmp_addr1( i_Start - prev_i_Start_tmp_addr1 + prev_i_End_tmp_addr1, j_Start - prev_j_Start_tmp_addr1 + prev_j_End_tmp_addr1, k_Start - prev_k_Start_tmp_addr1 + prev_k_End_tmp_addr1) == calAddrtmp_addr2(i_Start, j_Start, k_Start)) {
                rtHistoCal(prev_cnt_tmp_addr1);
                goto EndSample;
            }
        }
        if ( prev_cnt_tmp_addr2 != -1) {
            if ( calAddrtmp_addr2( i_Start - prev_i_Start_tmp_addr2 + prev_i_End_tmp_addr2, j_Start - prev_j_Start_tmp_addr2 + prev_j_End_tmp_addr2, k_Start - prev_k_Start_tmp_addr2 + prev_k_End_tmp_addr2) == calAddrtmp_addr2(i_Start, j_Start, k_Start)) {
                rtHistoCal(prev_cnt_tmp_addr2);
                goto EndSample;
            }
        }
