...
------Loop inc: (j + 1)
------Loop predicate: <
--------array access A.addr ((i * 1280) + k)
--------array access B.addr ((k * 1280) + j)
--------array access C.addr ((i * 1280) + j)
--------array access C.addr ((i * 1280) + j)
...
Dump tree:
----Sample number: 640
------Sample number: 409600
------Sample number: 409600
--------Sample number: 262144000
...
int calAddrB_addr3( int i, int k, int j) {
    int result = (((k * 1280) + j)) * 8 / 64;
    return result;
}
/* Array C_addr	i j */ 
/* i */
/* C_addr ((i * 1280) + j) 4 */
int calAddrC_addr4( int i, int k, int j) {
    int result = (((i * 1280) + j)) * 8 / 64;
    return result;
}
/* Array C_addr	i j */ 
/* i */
/* C_addr ((i * 1280) + j) 5 */
int calAddrC_addr5( int i, int k, int j) {
    int result = (((i * 1280) + j)) * 8 / 64;
    return result;
}
void ref_C_addr4(int thread_cnt, map<uint64_t, double> & RT) {
    /* Generating sampling loop */
    set<string> record;
    for ( int s = 0; s < 262144000;) {
SAMPLE:
        int i_Start = rand() % (1280 - 0) + 0;
        if (i_Start % 1 != 0) goto SAMPLE; 
        if (i_Start + thread_cnt * CHUNK_SIZE > 1280) { goto SAMPLE; }
        if ( (1280 - 0) == 0) goto SAMPLE;
        int k_Start = rand() % (1280 - 0) + 0;
        if (k_Start % 1 != 0) goto SAMPLE; 
        if ( (1280 - 0) == 0) goto SAMPLE;
        int j_Start = rand() % (1280 - 0) + 0;
        if (j_Start % 1 != 0) goto SAMPLE; 
        string idx_string =  to_string(i_Start) + "_" +  to_string(k_Start) + "_" +  to_string(j_Start) + "_" ;
        ...
        {
        int iLB0 = i_Start;
        for ( int i = iLB0; i < 1280; i=(i + 1)) {
            {
            int jLB1 = 0;
            for ( int j = jLB1; j < 1280; j=(j + 1)) {
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrC_addr0( i, j) == calAddrC_addr4(i_Start, k_Start, j_Start)) {
                            ...
                            function<uint64_t(uint64_t)> srcAddrCal = bind(calAddrC_addr4, _1, k_Start, j_Start);
                            function<uint64_t(uint64_t)> sinkAddrCal = bind(calAddrC_addr0, _1, j);
                            /* i 6556160 */
                            /* j 2 */
...
#endif
                            int reuse_type = -1;
                            uint64_t parallel_rt = parallel_predict((i_Start -0), (i - 0), cnt, 6556160, 6556160, middle_accesses, thread_cnt, true, true, reuse_type, srcAddrCal, sinkAddrCal);
...
#endif
#ifdef DEBUG
                                cout << "[C_addr4 --> C_addr0] [" << parallel_rt << "] (" << i_Start<< ", " << k_Start<< ", " << j_Start<< ") --> (" << i<< ", " << j<< ") " << endl;
#endif
                                // cout << "C_addr4;" << "(" << i_Start<< ", " << k_Start<< ", " << j_Start<< ");" << "C_addr0;" << "(" << i<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                        goto EndSample;
                    }
                }
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrC_addr1( i, j) == calAddrC_addr4(i_Start, k_Start, j_Start)) {
                            /* is_src_loop_outermost: 1 */
                            /* is_sink_loop_outermost: 1 */
                            /* is_normal_ref: true */
                            /* is_in_same_loop: true */
                            /* register the src/sink addr calculation function. Calling this function in other function do not have to know all index variables */
                            function<uint64_t(uint64_t)> srcAddrCal = bind(calAddrC_addr4, _1, k_Start, j_Start);
                            function<uint64_t(uint64_t)> sinkAddrCal = bind(calAddrC_addr1, _1, j);
                            /* i 6556160 */
                            /* j 2 */
                            /* compute the number of accesses between source and sink chunk */
                            uint64_t middle_accesses = 0;
...
#endif
#ifdef DEBUG
                                cout << "[C_addr4 --> C_addr1] [" << parallel_rt << "] (" << i_Start<< ", " << k_Start<< ", " << j_Start<< ") --> (" << i<< ", " << j<< ") " << endl;
#endif
                                // cout << "C_addr4;" << "(" << i_Start<< ", " << k_Start<< ", " << j_Start<< ");" << "C_addr1;" << "(" << i<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                        goto EndSample;
                    }
                }
            }
            ...
            for ( int k = kLB2; k < 1280; k=(k + 1)) {
                {
                int jLB3 = 0;
                if ( i == i_Start && k == k_Start ) {
                    jLB3 = j_Start;
                }
                for ( int j = jLB3; j < 1280; j=(j + 1)) {
                    if (cntStart == true) cnt++;
                    if (cntStart == true) cnt++;
                    if (cntStart == true) {
                        cnt++;
                        if ( calAddrC_addr4( i, k, j) == calAddrC_addr4(i_Start, k_Start, j_Start)) {
                                /* is_src_loop_outermost: 1 */
                                /* is_sink_loop_outermost: 1 */
                                /* is_normal_ref: true */
                                /* is_in_same_loop: true */
                                /* register the src/sink addr calculation function. Calling this function in other function do not have to know all index variables */
                                function<uint64_t(uint64_t)> srcAddrCal = bind(calAddrC_addr4, _1, k_Start, j_Start);
                                function<uint64_t(uint64_t)> sinkAddrCal = bind(calAddrC_addr4, _1, k, j);
                                /* i 6556160 */
                                /* k 5120 */
                                /* j 4 */
                                /* compute the number of accesses between source and sink chunk */
...
#endif
#ifdef DEBUG
                                    cout << "[C_addr4 --> C_addr4] [" << parallel_rt << "] (" << i_Start<< ", " << k_Start<< ", " << j_Start<< ") --> (" << i<< ", " << k<< ", " << j<< ") " << endl;
#endif
                                    // cout << "C_addr4;" << "(" << i_Start<< ", " << k_Start<< ", " << j_Start<< ");" << "C_addr4;" << "(" << i<< ", " << k<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                            goto EndSample;
                        }
                    }
                    cntStart = true;
                    if (cntStart == true) {
                        cnt++;
                        if ( calAddrC_addr5( i, k, j) == calAddrC_addr4(i_Start, k_Start, j_Start)) {
                                /* is_src_loop_outermost: 1 */
                                /* is_sink_loop_outermost: 1 */
                                /* is_normal_ref: true */
                                /* is_in_same_loop: true */
                                /* register the src/sink addr calculation function. Calling this function in other function do not have to know all index variables */
                                function<uint64_t(uint64_t)> srcAddrCal = bind(calAddrC_addr4, _1, k_Start, j_Start);
                                function<uint64_t(uint64_t)> sinkAddrCal = bind(calAddrC_addr5, _1, k, j);
                                /* i 6556160 */
                                /* k 5120 */
                                /* j 4 */
                                /* compute the number of accesses between source and sink chunk */
...
#endif
#ifdef DEBUG
                                    cout << "[C_addr4 --> C_addr5] [" << parallel_rt << "] (" << i_Start<< ", " << k_Start<< ", " << j_Start<< ") --> (" << i<< ", " << k<< ", " << j<< ") " << endl;
#endif
                                    // cout << "C_addr4;" << "(" << i_Start<< ", " << k_Start<< ", " << j_Start<< ");" << "C_addr5;" << "(" << i<< ", " << k<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                            goto EndSample;
                        }
                    }
                }
...
void ref_C_addr5(int thread_cnt, map<uint64_t, double> & RT) {
    /* Generating sampling loop */
    set<string> record;
    for ( int s = 0; s < 262144000;) {
SAMPLE:
        ...
        for ( int i = iLB0; i < 1280; i=(i + 1)) {
            ...
            for ( int j = jLB1; j < 1280; j=(j + 1)) {
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrC_addr0( i, j) == calAddrC_addr5(i_Start, k_Start, j_Start)) {
                            /* is_src_loop_outermost: 1 */
                            /* is_sink_loop_outermost: 1 */
                            /* is_normal_ref: true */
                            /* is_in_same_loop: true */
                            /* register the src/sink addr calculation function. Calling this function in other function do not have to know all index variables */
                            function<uint64_t(uint64_t)> srcAddrCal = bind(calAddrC_addr5, _1, k_Start, j_Start);
                            function<uint64_t(uint64_t)> sinkAddrCal = bind(calAddrC_addr0, _1, j);
                            /* i 6556160 */
                            /* j 2 */
                            /* compute the number of accesses between source and sink chunk */
...
#endif
#ifdef DEBUG
                                cout << "[C_addr5 --> C_addr0] [" << parallel_rt << "] (" << i_Start<< ", " << k_Start<< ", " << j_Start<< ") --> (" << i<< ", " << j<< ") " << endl;
#endif
                                // cout << "C_addr5;" << "(" << i_Start<< ", " << k_Start<< ", " << j_Start<< ");" << "C_addr0;" << "(" << i<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                        goto EndSample;
                    }
                }
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrC_addr1( i, j) == calAddrC_addr5(i_Start, k_Start, j_Start)) {
                            /* is_src_loop_outermost: 1 */
                            /* is_sink_loop_outermost: 1 */
                            /* is_normal_ref: true */
                            /* is_in_same_loop: true */
...
#endif
#ifdef DEBUG
                                cout << "[C_addr5 --> C_addr1] [" << parallel_rt << "] (" << i_Start<< ", " << k_Start<< ", " << j_Start<< ") --> (" << i<< ", " << j<< ") " << endl;
#endif
                                // cout << "C_addr5;" << "(" << i_Start<< ", " << k_Start<< ", " << j_Start<< ");" << "C_addr1;" << "(" << i<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                        goto EndSample;
                    }
                }
            }
            ...
            for ( int k = kLB2; k < 1280; k=(k + 1)) {
                ...
                for ( int j = jLB3; j < 1280; j=(j + 1)) {
                    ...
                    if (cntStart == true) {
                        cnt++;
                        if ( calAddrC_addr4( i, k, j) == calAddrC_addr5(i_Start, k_Start, j_Start)) {
                                /* is_src_loop_outermost: 1 */
                                /* is_sink_loop_outermost: 1 */
                                /* is_normal_ref: true */
                                /* is_in_same_loop: true */
...
#endif
#ifdef DEBUG
                                    cout << "[C_addr5 --> C_addr4] [" << parallel_rt << "] (" << i_Start<< ", " << k_Start<< ", " << j_Start<< ") --> (" << i<< ", " << k<< ", " << j<< ") " << endl;
#endif
                                    // cout << "C_addr5;" << "(" << i_Start<< ", " << k_Start<< ", " << j_Start<< ");" << "C_addr4;" << "(" << i<< ", " << k<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                            goto EndSample;
                        }
                    }
                    if (cntStart == true) {
                        cnt++;
                        if ( calAddrC_addr5( i, k, j) == calAddrC_addr5(i_Start, k_Start, j_Start)) {
                                /* is_src_loop_outermost: 1 */
                                /* is_sink_loop_outermost: 1 */
                                /* is_normal_ref: true */
                                /* is_in_same_loop: true */
...
#endif
#ifdef DEBUG
                                    cout << "[C_addr5 --> C_addr5] [" << parallel_rt << "] (" << i_Start<< ", " << k_Start<< ", " << j_Start<< ") --> (" << i<< ", " << k<< ", " << j<< ") " << endl;
#endif
                                    // cout << "C_addr5;" << "(" << i_Start<< ", " << k_Start<< ", " << j_Start<< ");" << "C_addr5;" << "(" << i<< ", " << k<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                            goto EndSample;
                        }
                    }
                    cntStart = true;
...
void ref_C_addr0(int thread_cnt, map<uint64_t, double> & RT) {
    /* Generating sampling loop */
    set<string> record;
    for ( int s = 0; s < 409600;) {
SAMPLE:
        int i_Start = rand() % (1280 - 0) + 0;
        if (i_Start % 1 != 0) goto SAMPLE; 
        if (i_Start + thread_cnt * CHUNK_SIZE > 1280) { goto SAMPLE; }
        ...
        for ( int i = iLB0; i < 1280; i=(i + 1)) {
            ...
            for ( int j = jLB1; j < 1280; j=(j + 1)) {
                if (cntStart == true) {
                    ...
                    if ( calAddrC_addr0( i, j) == calAddrC_addr0(i_Start, j_Start)) {
                            ...
                            /* is_in_same_loop: true */
                            /* register the src/sink addr calculation function. Calling this function in other function do not have to know all index variables */
                            function<uint64_t(uint64_t)> srcAddrCal = bind(calAddrC_addr0, _1, j_Start);
                            function<uint64_t(uint64_t)> sinkAddrCal = bind(calAddrC_addr0, _1, j);
                            /* i 6556160 */
                            /* j 2 */
                            /* compute the number of accesses between source and sink chunk */
...
#endif
#ifdef DEBUG
                                cout << "[C_addr0 --> C_addr0] [" << parallel_rt << "] (" << i_Start<< ", " << j_Start<< ") --> (" << i<< ", " << j<< ") " << endl;
#endif
                                // cout << "C_addr0;" << "(" << i_Start<< ", " << j_Start<< ");" << "C_addr0;" << "(" << i<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                        goto EndSample;
                    }
                }
                cntStart = true;
...
#endif
#ifdef DEBUG
                                cout << "[C_addr0 --> C_addr1] [" << parallel_rt << "] (" << i_Start<< ", " << j_Start<< ") --> (" << i<< ", " << j<< ") " << endl;
#endif
                                // cout << "C_addr0;" << "(" << i_Start<< ", " << j_Start<< ");" << "C_addr1;" << "(" << i<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                        goto EndSample;
                    }
                }
            }
            ...
            for ( int k = kLB2; k < 1280; k=(k + 1)) {
                ...
                for ( int j = jLB3; j < 1280; j=(j + 1)) {
                    ...
                    if (cntStart == true) {
                        cnt++;
                        if ( calAddrC_addr4( i, k, j) == calAddrC_addr0(i_Start, j_Start)) {
                                /* is_src_loop_outermost: 1 */
                                /* is_sink_loop_outermost: 1 */
                                /* is_normal_ref: true */
                                /* is_in_same_loop: true */
...
#endif
#ifdef DEBUG
                                    cout << "[C_addr0 --> C_addr4] [" << parallel_rt << "] (" << i_Start<< ", " << j_Start<< ") --> (" << i<< ", " << k<< ", " << j<< ") " << endl;
#endif
                                    // cout << "C_addr0;" << "(" << i_Start<< ", " << j_Start<< ");" << "C_addr4;" << "(" << i<< ", " << k<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                            goto EndSample;
                        }
                    }
                    if (cntStart == true) {
                        cnt++;
                        if ( calAddrC_addr5( i, k, j) == calAddrC_addr0(i_Start, j_Start)) {
                                /* is_src_loop_outermost: 1 */
                                /* is_sink_loop_outermost: 1 */
                                /* is_normal_ref: true */
                                /* is_in_same_loop: true */
...
#endif
#ifdef DEBUG
                                    cout << "[C_addr0 --> C_addr5] [" << parallel_rt << "] (" << i_Start<< ", " << j_Start<< ") --> (" << i<< ", " << k<< ", " << j<< ") " << endl;
#endif
                                    // cout << "C_addr0;" << "(" << i_Start<< ", " << j_Start<< ");" << "C_addr5;" << "(" << i<< ", " << k<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                            goto EndSample;
                        }
                    }
                }
        ...
        for ( int i = iLB0; i < 1280; i=(i + 1)) {
            ...
            for ( int j = jLB1; j < 1280; j=(j + 1)) {
                if (cntStart == true) {
                    ...
                    if ( calAddrC_addr0( i, j) == calAddrC_addr1(i_Start, j_Start)) {
                            ...
                            /* is_in_same_loop: true */
                            /* register the src/sink addr calculation function. Calling this function in other function do not have to know all index variables */
                            function<uint64_t(uint64_t)> srcAddrCal = bind(calAddrC_addr1, _1, j_Start);
                            function<uint64_t(uint64_t)> sinkAddrCal = bind(calAddrC_addr0, _1, j);
                            /* i 6556160 */
                            /* j 2 */
                            /* compute the number of accesses between source and sink chunk */
...
#endif
#ifdef DEBUG
                                cout << "[C_addr1 --> C_addr0] [" << parallel_rt << "] (" << i_Start<< ", " << j_Start<< ") --> (" << i<< ", " << j<< ") " << endl;
#endif
                                // cout << "C_addr1;" << "(" << i_Start<< ", " << j_Start<< ");" << "C_addr0;" << "(" << i<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                        goto EndSample;
                    }
                }
                if (cntStart == true) {
...
#endif
#ifdef DEBUG
                                cout << "[C_addr1 --> C_addr1] [" << parallel_rt << "] (" << i_Start<< ", " << j_Start<< ") --> (" << i<< ", " << j<< ") " << endl;
#endif
                                // cout << "C_addr1;" << "(" << i_Start<< ", " << j_Start<< ");" << "C_addr1;" << "(" << i<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                        goto EndSample;
                    }
                }
                cntStart = true;
            ...
            for ( int k = kLB2; k < 1280; k=(k + 1)) {
                ...
                for ( int j = jLB3; j < 1280; j=(j + 1)) {
                    ...
                    if (cntStart == true) {
                        cnt++;
                        if ( calAddrC_addr4( i, k, j) == calAddrC_addr1(i_Start, j_Start)) {
                                /* is_src_loop_outermost: 1 */
                                /* is_sink_loop_outermost: 1 */
                                /* is_normal_ref: true */
                                /* is_in_same_loop: true */
...
#endif
#ifdef DEBUG
                                    cout << "[C_addr1 --> C_addr4] [" << parallel_rt << "] (" << i_Start<< ", " << j_Start<< ") --> (" << i<< ", " << k<< ", " << j<< ") " << endl;
#endif
                                    // cout << "C_addr1;" << "(" << i_Start<< ", " << j_Start<< ");" << "C_addr4;" << "(" << i<< ", " << k<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                            goto EndSample;
                        }
                    }
                    if (cntStart == true) {
                        cnt++;
                        if ( calAddrC_addr5( i, k, j) == calAddrC_addr1(i_Start, j_Start)) {
                                /* is_src_loop_outermost: 1 */
                                /* is_sink_loop_outermost: 1 */
                                /* is_normal_ref: true */
                                /* is_in_same_loop: true */
...
#endif
#ifdef DEBUG
                                    cout << "[C_addr1 --> C_addr5] [" << parallel_rt << "] (" << i_Start<< ", " << j_Start<< ") --> (" << i<< ", " << k<< ", " << j<< ") " << endl;
#endif
                                    // cout << "C_addr1;" << "(" << i_Start<< ", " << j_Start<< ");" << "C_addr5;" << "(" << i<< ", " << k<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                            goto EndSample;
                        }
                    }
                }
                ...
                for ( int j = jLB3; j < 1280; j=(j + 1)) {
                    if (cntStart == true) {
                        ...
                        if ( calAddrA_addr2( i, k, j) == calAddrA_addr2(i_Start, k_Start, j_Start)) {
                                ...
                                /* is_in_same_loop: true */
                                /* register the src/sink addr calculation function. Calling this function in other function do not have to know all index variables */
                                function<uint64_t(uint64_t)> srcAddrCal = bind(calAddrA_addr2, _1, k_Start, j_Start);
                                function<uint64_t(uint64_t)> sinkAddrCal = bind(calAddrA_addr2, _1, k, j);
                                /* i 6556160 */
                                /* k 5120 */
                                /* j 4 */
                                /* compute the number of accesses between source and sink chunk */
...
#endif
#ifdef DEBUG
                                    cout << "[A_addr2 --> A_addr2] [" << parallel_rt << "] (" << i_Start<< ", " << k_Start<< ", " << j_Start<< ") --> (" << i<< ", " << k<< ", " << j<< ") " << endl;
#endif
                                    // cout << "A_addr2;" << "(" << i_Start<< ", " << k_Start<< ", " << j_Start<< ");" << "A_addr2;" << "(" << i<< ", " << k<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                            goto EndSample;
                        }
                    }
                    cntStart = true;
                ...
                for ( int j = jLB3; j < 1280; j=(j + 1)) {
                    ...
                    if (cntStart == true) {
                        ...
                        if ( calAddrB_addr3( i, k, j) == calAddrB_addr3(i_Start, k_Start, j_Start)) {
                                ...
                                /* is_in_same_loop: true */
                                /* register the src/sink addr calculation function. Calling this function in other function do not have to know all index variables */
                                function<uint64_t(uint64_t)> srcAddrCal = bind(calAddrB_addr3, _1, k_Start, j_Start);
                                function<uint64_t(uint64_t)> sinkAddrCal = bind(calAddrB_addr3, _1, k, j);
                                /* i 6556160 */
                                /* k 5120 */
                                /* j 4 */
                                /* compute the number of accesses between source and sink chunk */
...
#endif
                                int reuse_type = -1;
                                uint64_t parallel_rt = parallel_predict((i_Start -0), (i - 0), cnt, 6556160, 6556160, middle_accesses, thread_cnt, false, true, reuse_type, srcAddrCal, sinkAddrCal);
                                if (parallel_rt == 0) { goto EndSample; }
#ifdef DEBUG
                                rtHistoCal(RT, parallel_rt, 1.0);
#else
...
#endif
#ifdef DEBUG
                                    cout << "[B_addr3 --> B_addr3] [" << parallel_rt << "] (" << i_Start<< ", " << k_Start<< ", " << j_Start<< ") --> (" << i<< ", " << k<< ", " << j<< ") " << endl;
#endif
                                    // cout << "B_addr3;" << "(" << i_Start<< ", " << k_Start<< ", " << j_Start<< ");" << "B_addr3;" << "(" << i<< ", " << k<< ", " << j<< ");" << cnt << ";" << parallel_rt << ";" << reuse_type << endl;
                            goto EndSample;
                        }
                    }
                    cntStart = true;
...
void generate_per_thread_reuse(int thread_cnt) {
    ...
    ref_C_addr4(thread_cnt, tmpRT);
    ref_C_addr5(thread_cnt, tmpRT);
    ref_C_addr0(thread_cnt, tmpRT);
