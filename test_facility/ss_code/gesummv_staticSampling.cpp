...
/* for (i, 0, 1024) */
    ...
    while ( !candidate_thread_pool_1.empty()) {
        ...
        while ( !thread_pool.empty()) {
            ...
            cnt++;
            access = calAddrtmp_addr0( progress[t_select][0]);
        ...
        while ( !candidate_thread_pool_2.empty()) {
            ...
            while ( !thread_pool.empty()) {
                int t_select = thread_pool[rand() % thread_pool.size()];
                cnt++;
                access = calAddrtmp_addr1( progress[t_select][0], progress[t_select][1]);
                if (LAT.find(access) != LAT.end()) {
#ifdef DEBUG
            ...
            while ( !thread_pool.empty()) {
                int t_select = thread_pool[rand() % thread_pool.size()];
                cnt++;
                access = calAddrtmp_addr2( progress[t_select][0], progress[t_select][1]);
                if (LAT.find(access) != LAT.end()) {
#ifdef DEBUG
                    cout << "[REUSE of Addr " << access << "] " << cnt - get<0>(LAT[access]) << " find @(" << progress[t_select][0] << " "<< progress[t_select][1] << " "<< ")" << endl;
#endif
            ...
            cnt += THREAD_NUM;
            /* hasTNLoop: True */
            /* containsFirstTN: True */
            /* !containsFirstTN && hasTNLoop: False */
        ...
        while ( !thread_pool.empty()) {
            int t_select = thread_pool[rand() % thread_pool.size()];
            cnt++;
            access = calAddrtmp_addr3( progress[t_select][0]);
            if (LAT.find(access) != LAT.end()) {
#ifdef DEBUG
                cout << "[REUSE of Addr " << access << "] " << cnt - get<0>(LAT[access]) << " find @(" << progress[t_select][0] << " "<< ")" << endl;
#endif
...
/* for (j, 0, 1024) */
    ...
    while ( !candidate_thread_pool_1.empty()) {
        ...
        while ( !thread_pool.empty()) {
            int t_select = thread_pool[rand() % thread_pool.size()];
            cnt++;
            access = calAddry_addr0( progress[t_select][0]);
            if (LAT.find(access) != LAT.end()) {
#ifdef DEBUG
                cout << "[REUSE of Addr " << access << "] " << cnt - get<0>(LAT[access]) << " find @(" << progress[t_select][0] << " "<< ")" << endl;
#endif
        ...
        while ( !candidate_thread_pool_2.empty()) {
            ...
            while ( !thread_pool.empty()) {
                int t_select = thread_pool[rand() % thread_pool.size()];
                cnt++;
                access = calAddry_addr1( progress[t_select][0], progress[t_select][1]);
                if (LAT.find(access) != LAT.end()) {
#ifdef DEBUG
                    cout << "[REUSE of Addr " << access << "] " << cnt - get<0>(LAT[access]) << " find @(" << progress[t_select][0] << " "<< progress[t_select][1] << " "<< ")" << endl;
#endif
            ...
            while ( !thread_pool.empty()) {
                int t_select = thread_pool[rand() % thread_pool.size()];
                cnt++;
                access = calAddry_addr2( progress[t_select][0], progress[t_select][1]);
                if (LAT.find(access) != LAT.end()) {
#ifdef DEBUG
                    cout << "[REUSE of Addr " << access << "] " << cnt - get<0>(LAT[access]) << " find @(" << progress[t_select][0] << " "<< progress[t_select][1] << " "<< ")" << endl;
#endif
        ...
        while ( !thread_pool.empty()) {
            int t_select = thread_pool[rand() % thread_pool.size()];
            cnt++;
            access = calAddry_addr3( progress[t_select][0]);
            if (LAT.find(access) != LAT.end()) {
#ifdef DEBUG
                cout << "[REUSE of Addr " << access << "] " << cnt - get<0>(LAT[access]) << " find @(" << progress[t_select][0] << " "<< ")" << endl;
#endif
        ...
        while ( !thread_pool.empty()) {
            int t_select = thread_pool[rand() % thread_pool.size()];
            cnt++;
            access = calAddry_addr4( progress[t_select][0]);
            if (LAT.find(access) != LAT.end()) {
#ifdef DEBUG
                cout << "[REUSE of Addr " << access << "] " << cnt - get<0>(LAT[access]) << " find @(" << progress[t_select][0] << " "<< ")" << endl;
#endif
