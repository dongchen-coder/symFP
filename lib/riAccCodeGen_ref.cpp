...
namespace riAccCodeGen_ref {
    char AccLevelRICodeGen_ref::ID = 0;
    static RegisterPass<AccLevelRICodeGen_ref> X("riAccCodeGen_ref", "parallel random interleaving static sampling code generating pass (access based)", false, false);

    AccLevelRICodeGen_ref::AccLevelRICodeGen_ref() : FunctionPass(ID) {}

    void AccLevelRICodeGen_ref::numberRefToSameArray(loopAnalysis::LoopIndvBoundAnalysis::LoopRefTNode *LoopRefTree) {
    ...
    void AccLevelRICodeGen_ref::numberLoops(loopAnalysis::LoopIndvBoundAnalysis::LoopRefTNode *LoopRefTree) {
    ...
    void AccLevelRICodeGen_ref::initIndvName(loopAnalysis::LoopIndvBoundAnalysis::LoopRefTNode *LoopRefTree) {
    ...
    void AccLevelRICodeGen_ref::initArrayName() {
    ...
    void AccLevelRICodeGen_ref::addrCalFuncGen(loopAnalysis::LoopIndvBoundAnalysis::LoopRefTNode* LoopRefTree, std::vector<string> indvs) {
    ...
    void AccLevelRICodeGen_ref::addrCalFuncGenTop(loopAnalysis::LoopIndvBoundAnalysis::LoopRefTNode *LoopRefTree) {
    ...
    void AccLevelRICodeGen_ref::headerGen() {
...
#ifdef DumpRTMR
    void AccLevelRICodeGen_ref::rtHistoGen() {
...
#ifdef DumpRTMR
    void AccLevelRICodeGen_ref::rtDumpGen() {
    ...
    void AccLevelRICodeGen_ref::rtToMRGen() {
...
// 20MB
    ...
    void AccLevelRICodeGen_ref::mrDumpGen() {
    ...
    void AccLevelRICodeGen_ref::rtMergeGen() {
    ...
    void AccLevelRICodeGen_ref::GaussianDistrGen() {
    ...
    void AccLevelRICodeGen_ref::UniformDistrGen() {
    ...
    string AccLevelRICodeGen_ref::getBound(Value *bound) {
    ...
    string AccLevelRICodeGen_ref::getBound_Start(Value *bound) {
        if (isa<Instruction>(bound)) {
    ...
    std::vector<loopAnalysis::LoopIndvBoundAnalysis::LoopRefTNode*> AccLevelRICodeGen_ref::findLoops(
    ...
    void AccLevelRICodeGen_ref::LoopIterIncGen(
    ...
    bool AccLevelRICodeGen_ref::refRTSearchGen(
        loopAnalysis::LoopIndvBoundAnalysis::LoopRefTNode *LoopRefTree, 
        bool GenFlag,
        bool isFirstNestLoop,
        bool containsFirstTN,
        bool isFirstChildLoop,  
        std::string refName, 
        int useID, 
        std::vector<loopAnalysis::LoopIndvBoundAnalysis::LoopRefTNode*> loops, 
        vector<loopAnalysis::LoopIndvBoundAnalysis::LoopRefTNode*> currentLoops, 
        ...
        if (GenFlag == true) {
            ...
            if (!LoopRefTree->isThreadNode) {
                ...
                if (find(outloops.begin(), outloops.end(), LoopRefTree) != outloops.end()) { // out most loops
                    errs() << "#ifdef DEBUG\n";
        ...
        if (LoopRefTree->next != NULL) {
            if (!LoopRefTree->isThreadNode) {
                bool is_first_loop_child = false;
                for (std::vector<loopAnalysis::LoopIndvBoundAnalysis::LoopRefTNode*>::iterator it = LoopRefTree->next->begin(), eit = LoopRefTree->next->end(); it != eit; ++it) {
                    ...
                    is_first_loop_child = !((*it)->isThreadNode) && !is_first_loop_child;
                    if (GenFlag) { 
                        GenFlag = refRTSearchGen(*it, GenFlag, isFirstNestLoop, (!containsFirstTN && hasTNLoop), is_first_loop_child, refName, useID, loops, currentLoops_New, sampleIDVs, space + "    ");
                    } else {
                        GenFlag = refRTSearchGen(*it, GenFlag, isFirstNestLoop, (!containsFirstTN && hasTNLoop), is_first_loop_child, refName, useID, loops, currentLoops_New, sampleIDVs, space);
                    }
                    
                }
                /* hasTNLoop: contains TN in child node */
                if (hasTNLoop) {
                    errs() << space + "    /* hasTNLoop: ";
                    if (hasTNLoop) {
                        errs() << "True";
                    } else {
                        errs() <<  "False";
                    ...
                    errs() << " */\n";
                    errs() << space + "    /* !containsFirstTN && hasTNLoop: ";
                    if (!containsFirstTN && hasTNLoop) {
                        errs() << "True";
                    } else {
                        errs() << "False";
                    }
                    ...
                    errs() << space + "    for (int t_select = 0; t_select < THREAD_NUM; t_select++) {\n"; 
                    LoopIterIncGen(LoopRefTree, currentLoops, space + "        ", !containsFirstTN);
                    errs() << space + "    }\n";
                    errs() << space + "} // end of while loop\n";
                    if (containsFirstTN) {
                        errs() << space + "for (int tid = 0; tid < THREAD_NUM; tid++) {\n";
                        errs() << space + "    progress[tid].pop_back();\n";
                        errs() << space + "}\n";
                    }
                }
            }
    ...
    void AccLevelRICodeGen_ref::refRTBodyGen(loopAnalysis::LoopIndvBoundAnalysis::LoopRefTNode *LoopRefTree, string refName, int useID) {
        ...
        if (loops.size() == 0) {
            ...
            errs() << "\n";
            refRTSearchGen(LoopRefTree, false, true, false, true, refName, useID, loops, vector<loopAnalysis::LoopIndvBoundAnalysis::LoopRefTNode *>(), sampleIDVs, space);
            errs() << space + "s++;\n";
            errs() << space + "}\n";
            return;
        }
        ...
        refRTSearchGen(LoopRefTree, false, true, false, true, refName, useID, loops, vector<loopAnalysis::LoopIndvBoundAnalysis::LoopRefTNode *>(), sampleIDVs, "    ");
        return;
    }
    
    void AccLevelRICodeGen_ref::refRTGen(loopAnalysis::LoopIndvBoundAnalysis::LoopRefTNode *LoopRefTree) {
        vector<string> visit;
    ...
    void AccLevelRICodeGen_ref::mainGen() {
    ...
    bool AccLevelRICodeGen_ref::runOnFunction(Function &F) {
    ...
    void AccLevelRICodeGen_ref::getAnalysisUsage(AnalysisUsage &AU) const {
        AU.setPreservesAll();
