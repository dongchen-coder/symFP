...
namespace ssCodeGen_ref {
	...
	void StaticSamplingCodeGen_ref::IterationStructGen() 
	{
		errs() << "struct Iteration {\n";
		errs() << "    int LoopID;\n";
		errs() << "	   vector<int> IV;\n";
		errs() << "	   Iteration()\n";
		errs() << "	   {\n";
		errs() << "        LoopID = 0;\n";
		errs() << "	   }\n";
		errs() << "    Iteration(vector<int> iv)\n";
		errs() << "    {\n";
		errs() << "        LoopID = 0;\n";
		errs() << "        IV = iv;\n";
		errs() << "    }\n";
		errs() << "    Iteration(int lid, vector<int> iv)\n";
		errs() << "    {\n";
		errs() << "        LoopID = lid;\n";
		errs() << "        IV = iv;\n";
		errs() << "    }\n";
		errs() << "    string toString()\n";
		errs() << "    {\n";
		errs() << "        string ret = \"(\";\n";
		errs() << "        for (auto i : IV) {\n";
		errs() << "            ret += (to_string(i) + \",\");\n";
		errs() << "        }\n";
		errs() << "        ret.pop_back();\n";
		errs() << "        ret += \")\";\n";
		errs() << "        return ret;\n";
		errs() << "    }\n";
		errs() << "    bool operator < (const Iteration& iter) const\n";
		errs() << "    {\n";
		errs() << "        if (LoopID != iter.LoopID)\n";
		errs() << "            return LoopID < iter.LoopID;\n";
		errs() << "        int common = (int) min(IV.size(), iter.IV.size());\n";
		errs() << "        for (int i = 0; i < common; i++) {\n";
		errs() << "            if (IV[i] != iter.IV[i])\n";
		errs() << "            return IV[i] < iter.IV[i];\n";
		errs() << "        }\n";
		errs() << "        return false;\n";
		errs() << "    }\n";
		errs() << "    bool operator > (const Iteration& iter) const\n";
		...
		errs() << "    }\n";
		errs() << "};\n";
		errs() << "struct IterationRange {\n";
		errs() << "    Iteration start;\n";
		errs() << "    Iteration end;\n";
		errs() << "    IterationRange() {}\n";
		errs() << "    IterationRange(Iteration src, Iteration sink)\n";
		errs() << "    {\n";
		errs() << "        start = src;\n";
		errs() << "        end = sink;\n";
		errs() << "    }\n";
		errs() << "    bool operator < (const IterationRange& iterRange) const\n";
		errs() << "    {\n";
		errs() << "        return start < iterRange.start;\n";
		errs() << "    }\n";
		errs() << "    bool mergable(IterationRange IR)\n";
		errs() << "    {\n";
		errs() << "        if (start > IR.end || end < IR.start)\n";
		errs() << "            return false;\n";
		errs() << "        return true;\n";
		errs() << "    }\n";
		errs() << "    void merge(IterationRange IR)\n";
		errs() << "    {\n";
		errs() << "        if (start > IR.start) {\n";
		errs() << "            start = IR.start;\n";
		errs() << "        }\n";
		errs() << "        if (end < IR.end) {\n";
		errs() << "            end = IR.end;\n";
		...
		errs() << "    void dump()\n";
		errs() << "    {\n";
		errs() << "        cout << start.toString() << \", \" << end.toString() << endl;\n";
		errs() << "    }\n";
		errs() << "};\n";
		errs() << "vector<IterationRange> IRVec;\n";
	}
			...
			if (LoopRefTree->AA != NULL) {
				...
				if (arrayName[LoopRefTree->AA] == refName) {
					...
					errs() << tmp + ")) {\n";
						...
						errs() << "});\n";
						errs() << space + "            IRVec.emplace_back(IterationRange(start, end));\n";
                        */
...
//                    if ( find(loops.back()->next->begin(), loops.back()->next->end(), LoopRefTree) == loops.back()->next->end())  {
	...
	void StaticSamplingCodeGen_ref::refRTBodyGen(loopAnalysis::LoopIndvBoundAnalysis::LoopRefTNode *LoopRefTree, string refName, int useID) {
		...
		string iteration_variable_string = "";
        /*
		errs() << space << "    Iteration start({";
		for ( vector<loopAnalysis::LoopIndvBoundAnalysis::LoopRefTNode*>::iterator lit = loops.begin(), elit = loops.end(); lit != elit; ++lit) {
			for (unsigned long i = 0; i < (*lit)->LIS->IDV->size(); i++) {
				iteration_variable_string += indvName[(*(*lit)->LIS->IDV)[i]] + "_Start,";
			}
		}
		iteration_variable_string.pop_back(); // remove the last ,
		errs() << iteration_variable_string << "});\n";
		*/
...
//                errs() << " cout << \" End " + it->first +  to_string(i) + " \";\n " ;
	...
	void StaticSamplingCodeGen_ref::mainGen() {
		...
		// errs() << "    dumpMR();\n";
        /*
		errs() << "    sort(IRVec.begin(), IRVec.end());\n";
		errs() << "    vector<IterationRange>::iterator iter = IRVec.begin();\n";
		errs() << "    vector<IterationRange>::iterator next = iter+1;\n";
		errs() << "    while (next != IRVec.end()) {\n";
		errs() << "        if (iter->mergable(*next)) {\n";
		errs() << "            iter->merge(*next);\n";
		errs() << "            next = IRVec.erase(next);\n";
		errs() << "        } else {\n";
		errs() << "            iter = next;\n";
		errs() << "            next = iter+1;\n";
		errs() << "        }\n";
		errs() << "    }\n";
		errs() << "    iter = IRVec.begin();\n";
		errs() << "    while(iter != IRVec.end()) {\n";
		errs() << "        iter->dump();\n";
		errs() << "        iter++;\n";
		errs() << "    }\n";
        */
