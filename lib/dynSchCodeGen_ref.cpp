...
namespace dynSchCodeGen_ref {
	char DynamicSchedulingSamplerCodeGen::ID = 0;
	static RegisterPass<DynamicSchedulingSamplerCodeGen> X("dynSchCodeGen_ref", "parallel random interleaving static sampling code generating pass (both reference and iteration)", false, false);

	DynamicSchedulingSamplerCodeGen::DynamicSchedulingSamplerCodeGen() : FunctionPass(ID) {}

	void DynamicSchedulingSamplerCodeGen::numberRefToSameArray(LoopRefTNode *LoopRefTree) {
	...
	void DynamicSchedulingSamplerCodeGen::numberLoops(LoopRefTNode *LoopRefTree) {
	...
	void DynamicSchedulingSamplerCodeGen::initIndvName(LoopRefTNode *LoopRefTree) {
	...
	void DynamicSchedulingSamplerCodeGen::initArrayName() {
	...
	void DynamicSchedulingSamplerCodeGen::addrCalFuncGen(LoopRefTNode* LoopRefTree, std::vector<string> indvs) {
	...
	void DynamicSchedulingSamplerCodeGen::addrCalFuncGenTop(LoopRefTNode *LoopRefTree) {
	...
	void DynamicSchedulingSamplerCodeGen::headerGen() {
		...
		errs() << "    int chunk_size;\n";
		errs() << "    int trip;\n";
		errs() << "    int avail_chunk;\n";
		errs() << "    unordered_map<int, Chunk> history;\n";
		errs() << "public:\n";
		errs() << "    ChunkEngine() {} \n";
		...
		for (std::map<Instruction*, int>::iterator it = refNumber.begin(), eit = refNumber.end(); it != eit; ++it) {
			...
			errs() << "std::map<uint64_t, uint64_t> LAT_" + arrayName[it->first] + ";\n";
		}
...
#ifdef DumpRTMR
	void DynamicSchedulingSamplerCodeGen::rtHistoGen() {
...
//		errs() << "    lck.unlock();\n";
	...
	/* Generate the function to calculate the bins */
	void DynamicSchedulingSamplerCodeGen::subBlkRTGen() {
		string space = "    ";
...
#ifdef DumpRTMR
	void DynamicSchedulingSamplerCodeGen::rtDumpGen() {
	...
	void DynamicSchedulingSamplerCodeGen::rtToMRGen() {
...
// 20MB
	...
	void DynamicSchedulingSamplerCodeGen::mrDumpGen() {
	...
	void DynamicSchedulingSamplerCodeGen::rtMergeGen() {
	...
	void DynamicSchedulingSamplerCodeGen::GaussianDistrGen() {
	...
	void DynamicSchedulingSamplerCodeGen::UniformDistrGen() {
	...
	string DynamicSchedulingSamplerCodeGen::getBound(Value *bound) {
	...
	string DynamicSchedulingSamplerCodeGen::getBound_Start(Value *bound) {
		if (isa<Instruction>(bound)) {
	...
	std::vector<LoopRefTNode*> DynamicSchedulingSamplerCodeGen::findLoops(
	...
	DepParentInfo DynamicSchedulingSamplerCodeGen::getParent(Value * v)
	{
	...
	void DynamicSchedulingSamplerCodeGen::LoopIterIncGen(AccGraph *G,
															AccGraphEdge *E,
	...
	void DynamicSchedulingSamplerCodeGen::LoopIterUpdateGen(AccGraph *G,
															AccGraphEdge *E,
															LoopRefTNode* IDomLoop,
															string space,
															...
															bool EnableIterationUpdate)
	{
		...
		if (EnableIterationUpdate) {
			...
			// when pushing LB, it should do from outermost to innermsot
			vector<LoopRefTNode *> L1, L2;
			vector<LoopRefTNode *>::iterator loopIter;
			vector<LoopRefTNode *>::reverse_iterator rloopIter;
			...
			for (rloopIter = L2.rbegin(); rloopIter != L2.rend(); ++rloopIter) {
				string lower_bound = getBound((*rloopIter)->LIS->LB->front().first);
				DepParentInfo pinfo = getParent((*rloopIter)->LIS->IDV->front());
				if (pinfo.first && pinfo.second > 0) {
...
//		}
	...
	void DynamicSchedulingSamplerCodeGen::reuseUpdateGen(LoopRefTNode *access, int currentAccessLoopNestCnt, string accessName, string space)
	{
		errs() << space << "cnt++;\n";
	...
	bool DynamicSchedulingSamplerCodeGen::refRTSearchGen(
		LoopRefTNode *root,
		LoopRefTNode *LoopRefTree,
		...
		if (find(outloops.begin(), outloops.end(), LoopRefTree) != outloops.end()) {
			...
			while (true) {
				...
				if (GraphIter == NodesInGraph.begin()) {
					errs() << space << "ChunkEngine engine(chunk_size, (" << getBound(LoopRefTree->LIS->LB->front().second) << " - " << getBound(LoopRefTree->LIS->LB->front().first) << "));\n";
					...
					errs() << space << space << space << space << "Progress p(\"" + arrayName[currentNode->AA] + to_string(refNumber[currentNode->AA]) << "\", iteration_vector, c);\n";
...
//			errs() << space << space << space << "if (!progress[tid_to_run].isInBound()) {\n";
	...
	void DynamicSchedulingSamplerCodeGen::refRTBodyGen(LoopRefTNode *LoopRefTree) {
	...
	void DynamicSchedulingSamplerCodeGen::perArrayRTBodyGen(std::string array, LoopRefTNode *LoopRefTree)
	{
		string space = "    ";
		errs() << space << "uint64_t cnt = 0;\n";
		errs() << space << "int tid_to_run, chunk_size;\n";
	...
	bool DynamicSchedulingSamplerCodeGen::perArrayRTSearchGen(string array,
		LoopRefTNode *root,
		LoopRefTNode *LoopRefTree,
		vector<LoopRefTNode*> currentLoops,
		string space
	...
	void DynamicSchedulingSamplerCodeGen::refRTGen(LoopRefTNode *LoopRefTree) {
...
//		errs() << "}\n";
	...
	void DynamicSchedulingSamplerCodeGen::mainGen() {
...
//		errs() << "    interleaving();\n";
	...
	bool DynamicSchedulingSamplerCodeGen::runOnFunction(Function &F) {
	...
	void DynamicSchedulingSamplerCodeGen::getAnalysisUsage(AnalysisUsage &AU) const {
		AU.setPreservesAll();
