...
for rate in rates:
	...
	path = "../test/polyBench/result/random_ss_"+str(rate) +"/"
		...
		if "result.txt" in f:
			...
			for line in File:
				...
				if (lineTmp[0].isdigit()):
					ss_c[rate][FileName].append(float(lineTmp[0]) * 8 / 1024)
					ss_mr[rate][FileName].append(float(lineTmp[1]))
...
for key in trace_c.keys():
	for rate in rates:
		if (key in ss_c[rate].keys() ):

			plt.plot(ss_c[rate][key], ss_mr[rate][key], label = 'Static sampling (Random sampling rate '+ str(rate) + ')')
