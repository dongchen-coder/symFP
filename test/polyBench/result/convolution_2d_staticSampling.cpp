...
int calAddrA_addr0( int i, int j) {
    int result = ((((i - 1) * 1024) + (j - 1))) * 8 / 32;
    return result;
}
int calAddrA_addr1( int i, int j) {
    int result = ((((i + 0) * 1024) + (j - 1))) * 8 / 32;
    return result;
}
...
int calAddrA_addr3( int i, int j) {
    int result = ((((i - 1) * 1024) + (j + 0))) * 8 / 32;
    return result;
}
int calAddrA_addr4( int i, int j) {
    int result = ((((i + 0) * 1024) + (j + 0))) * 8 / 32;
    return result;
}
...
void ref_A_addr0() {
    ...
    for ( int s = 0; s < 400;) {
        ...
        while ( record.find(idx_string) != record.end() ) {
            i_Start = rand() % (1023 - 1) + 1;
            j_Start = rand() % (1023 - 1) + 1;
        ...
        bool cntStart = false;
        int iLB0 = i_Start;
        for ( int i = iLB0; i < 1023; i++) {
            int jLB1 = 1;
            if ( i == i_Start ) {
                jLB1 = j_Start;
            }
            for ( int j = jLB1; j < 1023; j++) {
                if (cntStart == true) {
                    cnt++;
                ...
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr6( i, j) == calAddrA_addr0(i_Start, j_Start)) {
                        rtHistoCal(cnt);
                        goto EndSample;
                    }
                }
    ...
    for ( int s = 0; s < 400;) {
        ...
        for ( int i = iLB0; i < 1023; i++) {
            ...
            for ( int j = jLB1; j < 1023; j++) {
                ...
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr6( i, j) == calAddrA_addr1(i_Start, j_Start)) {
                        rtHistoCal(cnt);
                        goto EndSample;
                    }
                }
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr7( i, j) == calAddrA_addr1(i_Start, j_Start)) {
                        rtHistoCal(cnt);
                        goto EndSample;
                    }
                }
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr8( i, j) == calAddrA_addr1(i_Start, j_Start)) {
                        rtHistoCal(cnt);
                        goto EndSample;
                    }
                }
    ...
    for ( int s = 0; s < 400;) {
        ...
        for ( int i = iLB0; i < 1023; i++) {
            ...
            for ( int j = jLB1; j < 1023; j++) {
                ...
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr6( i, j) == calAddrA_addr2(i_Start, j_Start)) {
                        rtHistoCal(cnt);
                        goto EndSample;
                    }
                }
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr7( i, j) == calAddrA_addr2(i_Start, j_Start)) {
                        rtHistoCal(cnt);
                        goto EndSample;
                    }
                }
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr8( i, j) == calAddrA_addr2(i_Start, j_Start)) {
                        rtHistoCal(cnt);
                        goto EndSample;
                    }
                }
    ...
    for ( int s = 0; s < 400;) {
        ...
        for ( int i = iLB0; i < 1023; i++) {
            ...
            for ( int j = jLB1; j < 1023; j++) {
                ...
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr6( i, j) == calAddrA_addr3(i_Start, j_Start)) {
                        rtHistoCal(cnt);
                        goto EndSample;
                    }
                }
    ...
    for ( int s = 0; s < 400;) {
        ...
        for ( int i = iLB0; i < 1023; i++) {
            ...
            for ( int j = jLB1; j < 1023; j++) {
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr0( i, j) == calAddrA_addr6(i_Start, j_Start)) {
                        rtHistoCal(cnt);
                        goto EndSample;
                    }
                }
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr1( i, j) == calAddrA_addr6(i_Start, j_Start)) {
                        rtHistoCal(cnt);
                        goto EndSample;
                    }
                }
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr2( i, j) == calAddrA_addr6(i_Start, j_Start)) {
                        rtHistoCal(cnt);
                        goto EndSample;
                    }
                }
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr3( i, j) == calAddrA_addr6(i_Start, j_Start)) {
                        rtHistoCal(cnt);
                        goto EndSample;
                    }
                }
                ...
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr7( i, j) == calAddrA_addr6(i_Start, j_Start)) {
                        rtHistoCal(cnt);
                        goto EndSample;
                    }
                }
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr8( i, j) == calAddrA_addr6(i_Start, j_Start)) {
                        rtHistoCal(cnt);
                        goto EndSample;
                    }
                }
    ...
    for ( int s = 0; s < 400;) {
        ...
        for ( int i = iLB0; i < 1023; i++) {
            ...
            for ( int j = jLB1; j < 1023; j++) {
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr0( i, j) == calAddrA_addr7(i_Start, j_Start)) {
                        rtHistoCal(cnt);
                        goto EndSample;
                    }
                }
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr1( i, j) == calAddrA_addr7(i_Start, j_Start)) {
                        rtHistoCal(cnt);
                        goto EndSample;
                    }
                }
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr2( i, j) == calAddrA_addr7(i_Start, j_Start)) {
                        rtHistoCal(cnt);
                        goto EndSample;
                    }
                }
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr3( i, j) == calAddrA_addr7(i_Start, j_Start)) {
                        rtHistoCal(cnt);
                        goto EndSample;
                    }
                }
                ...
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr5( i, j) == calAddrA_addr7(i_Start, j_Start)) {
                        rtHistoCal(cnt);
                        goto EndSample;
                    }
                }
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr6( i, j) == calAddrA_addr7(i_Start, j_Start)) {
                        rtHistoCal(cnt);
                        goto EndSample;
                    }
                }
                ...
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr8( i, j) == calAddrA_addr7(i_Start, j_Start)) {
                        rtHistoCal(cnt);
                        goto EndSample;
                    }
                }
    ...
    for ( int s = 0; s < 400;) {
        ...
        for ( int i = iLB0; i < 1023; i++) {
            ...
            for ( int j = jLB1; j < 1023; j++) {
                ...
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr1( i, j) == calAddrA_addr8(i_Start, j_Start)) {
                        rtHistoCal(cnt);
                        goto EndSample;
                    }
                }
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr2( i, j) == calAddrA_addr8(i_Start, j_Start)) {
                        rtHistoCal(cnt);
                        goto EndSample;
                    }
                }
                ...
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr6( i, j) == calAddrA_addr8(i_Start, j_Start)) {
                        rtHistoCal(cnt);
                        goto EndSample;
                    }
                }
                if (cntStart == true) {
                    cnt++;
                    if ( calAddrA_addr7( i, j) == calAddrA_addr8(i_Start, j_Start)) {
                        rtHistoCal(cnt);
                        goto EndSample;
                    }
                }
